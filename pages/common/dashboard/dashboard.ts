...
async selectShipFromShipSelectionPicklist(shipName: string) {
    ...
    console.log("Step 1: Wait loader properly...");
    await waitForLoaderToDisappear(this.page);

    console.log("Step 2: Click picklist...");
    await picklist.click();

    console.log("Step 3: Wait for search input...");
    await searchInput.waitFor({ state: 'visible', timeout: 60000 });

    console.log("Step 4: Search ship...");
    await searchInput.fill(shipName);
    await searchInput.press('Enter');

    console.log("Step 5: Select ship...");
    await this.page.locator(shipSelectionPicklist.SHIP_SELECTION_FIRST_RESULT, {
        hasText: shipName
    }).click();

    const context = this.page.context();

    console.log("Step 6: Click OK & WAIT for new tab (STRICT)...");
...
async clickOnNetworkOkButtonIfExists() {
    try {
        ...
        console.log("Step 1: Wait DOM...");
        await this.page.waitForLoadState('domcontentloaded').catch(() => {});

        // 🔥 Step 2: Try detect alert (short window)
        ...
        // 🔥 Step 3: Wait loader (longer for slow app)
        await loader.waitFor({ state: 'hidden', timeout: 20000 }).catch(() => {
            console.log("⚠ Loader still visible, continuing...");
        });

        // 🔥 Step 4: Handle alert
        if (alertDetected && await alert.isVisible()) {
            console.log("✅ Clicking alert");
            await alert.click({ noWaitAfter: true });
        } else {
            console.log("ℹ No alert or already gone");
        }

    } catch (e) {
        console.log("Handled safely:", e);
    }
}
    // async clickOnNetworkOkButtonIfExists() {
    //     try {
    ...
    //         await loader.waitFor({ state: 'hidden', timeout: 3000000 }).catch(() => { });
    ...
    //     const context = this.page.context();      
    //     console.log("Step 6: Click OK & WAIT FOR NEW TAB (STRICT)...");
    //     // 🔥 Start waiting BEFORE click (important)
    //     const popupPromise = context.waitForEvent('page', { timeout: 60000 }); // increase if needed
    //     await this.page.click(shipSelectionPicklist.SHIP_SELECTION_OK_BUTTON, {noWaitAfter: true});      
    //     // 🔥 If no tab → this WILL throw (good, we want strict behavior)
    //     const newPage = await popupPromise;      
    //     console.log("✅ New tab captured");      
    //     await newPage.waitForLoadState('domcontentloaded');
    //     await newPage.bringToFront();
